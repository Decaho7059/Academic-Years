	...
	// Implementation for RealEstate interface
	...
	func main() {
		...
		listings := []RealEstate{
		  ...
		  &House{ListingInfo{"Maplewood 889", 850000, false}, 
										  Rectangle{50.0, 110.0}}}
		// Create a seller for every listing
		sellers := []*Seller{
			...
			go func(buy *Buyer) {
				for amount, valid := buy.nextBid(); valid; amount, valid = buy.nextBid() {
					...
					for _, s := range sellers {
						...
						s.OfferChan <- amount
						success := <-s.ResponseChan
						// Bid accepted close deal
						if success {
		...
		// Ensure that we only exit if concurrent bidding process is complete
